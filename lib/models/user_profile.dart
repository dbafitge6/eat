class UserProfile {
  final double heightCm;
  final double weightKg;
  final int age;
  final int sex; // 0=男 1=女
  final int activityLevel; // 0=低 1=普通 2=高
  final int goal; // 0=減量 1=維持 2=増量
  final double targetKcal;
  final double targetWaterMl;

  const UserProfile({
    required this.heightCm,
    required this.weightKg,
    required this.age,
    required this.sex,
    required this.activityLevel,
    required this.goal,
    required this.targetKcal,
    required this.targetWaterMl,
  });

  double get bmi => weightKg / ((heightCm / 100) * (heightCm / 100));

  double get bmr {
    if (sex == 0) {
      return 13.397 * weightKg + 4.799 * heightCm - 5.677 * age + 88.362;
    } else {
      return 9.247 * weightKg + 3.098 * heightCm - 4.330 * age + 447.593;
    }
  }

  static double calcTargetKcal(double bmr, int activityLevel, int goal) {
    final activityFactors = [1.2, 1.55, 1.725];
    final tdee = bmr * activityFactors[activityLevel];
    switch (goal) {
      case 0:
        return tdee - 500;
      case 2:
        return tdee + 300;
      default:
        return tdee;
    }
  }

  factory UserProfile.fromMap(Map<String, dynamic> m) => UserProfile(
        heightCm: (m['height_cm'] as num).toDouble(),
        weightKg: (m['weight_kg'] as num).toDouble(),
        age: m['age'] as int,
        sex: m['sex'] as int,
        activityLevel: m['activity_level'] as int,
        goal: m['goal'] as int,
        targetKcal: (m['target_kcal'] as num).toDouble(),
        targetWaterMl: (m['target_water_ml'] as num).toDouble(),
      );

  Map<String, dynamic> toMap() => {
        'height_cm': heightCm,
        'weight_kg': weightKg,
        'age': age,
        'sex': sex,
        'activity_level': activityLevel,
        'goal': goal,
        'target_kcal': targetKcal,
        'target_water_ml': targetWaterMl,
      };
}
